...
# ==============================================================================
# 1. AMOS22 UDA CONFIGURATION (MR -> CT)
# ==============================================================================
CONFIG = {
    ...
    # 🌟 LOAD AMOS MR SUPERVISED BASELINE (5-Class)
    'PRETRAINED_SEGMENTOR': r'D:\Nilesh\AMOS22\Supervised_MR_Baseline\best_model_AMOS_MR.pth',
    'SAVE_DIR': r'D:\Nilesh\AMOS22\UDA_MR2CT_Phase1', 
    ...
    'FDA_BETA': 0.05,
    'CONFIDENCE_THRESH': 0.85   # CT Target -> Strict Threshold
}
...
class DBSNet(nn.Module):
    ...
    def forward(self, x):
        ...
        # 🌟 THE FIX: Ensure bottleneck is returned to prevent D_feat dimension crashes
...
class SoftmaxWeightedLoss(nn.Module):
    ...
    def forward(self, pred_body, pred_edge, pred_final, mask_long, mask_onehot, edge, body):
        ...
        # 🌟 THE FIX: Return a single tensor to avoid unpacking crashes
...
def save_qualitative_results(G, S, dataset, device, save_dir, epoch):
    ...
    ax[1,0].imshow(r_ct, cmap='gray'); ax[1,0].set_title("Real CT (AMOS22 Unseen)")
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, f"AMOS22_Qualitative_{epoch}.png"), dpi=300)
    plt.close()
    G.train(); S.train()

# ==============================================================================
# 8. MAIN EXECUTION (MR -> CT Phase 1)
# ==============================================================================
if __name__ == '__main__':
    print(f"\n--- Initializing AMOS22 UDA SOTA (MR -> CT) ---")
    ...
    if not os.path.exists(CONFIG['PRETRAINED_SEGMENTOR']):
        print(f"⚠️ ERROR: Could not find Supervised MR Baseline at {CONFIG['PRETRAINED_SEGMENTOR']}")
        print("Please run the supervised training script on AMOS MR first!")
        exit(1)
        
    print(f"Loading Pre-trained AMOS MR Segmentor: {os.path.basename(CONFIG['PRETRAINED_SEGMENTOR'])}")
    ...
    for epoch in range(CONFIG['EPOCHS']):
        ...
        for batch_idx, (s_img, s_img_fda, s_mask_long, s_mask_onehot, s_edge, s_body, t_img) in enumerate(tqdm(train_loader, leave=False, desc=f"Epoch {epoch+1}")):
            ...
            # 🌟 Single-tensor assignment for proper unpacking
            ...
            with torch.no_grad():
                ...
                # Hard Pseudo-Labels via Argmax -> One-Hot
            ...
            # AMOS CLASS WEIGHTING: (Liver:1x, RKid:5x, LKid:5x, Spleen:2x)
            class_weights = torch.tensor([1.0, 5.0, 5.0, 2.0], device=device).view(1, 4, 1, 1)
            
            # Weighted Masked CBST Loss
            ...
            # 🌟 MATHEMATICALLY BOUNDED DICE LOSS FIX
        ...
        if v_dice > best_dice:
            best_dice = v_dice
            torch.save(S.state_dict(), os.path.join(CONFIG['SAVE_DIR'], "best_AMOS_Segmentor_MR2CT.pth"))
            torch.save(G.state_dict(), os.path.join(CONFIG['SAVE_DIR'], "best_AMOS_Generator_MR2CT.pth"))
    ...
    print("\n" + "="*70)
    print("🚀 FINAL ZERO-SHOT TEST RESULTS (AMOS MR -> CT)")
    print("="*70)
    
    test_ds = UnpairedUDADataset(None, None, *filter_t(t_test_c, True), img_size=CONFIG['IMG_SIZE'])
    test_loader = DataLoader(test_ds, batch_size=1, shuffle=False)
    
    S.load_state_dict(torch.load(os.path.join(CONFIG['SAVE_DIR'], "best_AMOS_Segmentor_MR2CT.pth"), weights_only=True))
