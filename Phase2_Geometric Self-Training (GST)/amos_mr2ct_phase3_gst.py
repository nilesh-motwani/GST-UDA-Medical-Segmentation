...
# ==============================================================================
# 1. AMOS22 CONFIGURATION (MR -> CT PHASE 2)
# ==============================================================================
CONFIG = {
    'SOURCE_IMG_DIR': r'D:\Nilesh\AMOS22\AMOS22_Data_2D_Fixed\Images\MR',
    'SOURCE_MASK_DIR': r'D:\Nilesh\AMOS22\AMOS22_Data_2D_Fixed\Masks\MR',
    'TARGET_IMG_DIR': r'D:\Nilesh\AMOS22\AMOS22_Data_2D_Fixed\Images\CT',
    'TARGET_MASK_DIR': r'D:\Nilesh\AMOS22\AMOS22_Data_2D_Fixed\Masks\CT',
    
    # 🌟 LOAD PHASE 1 WEIGHTS (Warm start from the recovered Epoch 83 weights)
    'PRETRAINED_SEGMENTOR': r'D:\Nilesh\AMOS22\UDA_MR2CT_Phase1\best_AMOS_Segmentor_MR2CT.pth',
    'SAVE_DIR': r'D:\Nilesh\AMOS22\22 July\UDA_MR2CT_Phase2_GST',
    
    # 🌟 50-EPOCH SPRINT FOR BOUNDARY SHARPENING
    'EPOCHS': 50,               
    'BATCH_SIZE': 16,             
    'LR_GAN': 2e-4,              
    'LR_SEG': 1e-4,              
    ...
    'FDA_BETA': 0.05,
    'CONFIDENCE_THRESH': 0.85   # CT Target -> Strict boundary threshold
}
...
class UnpairedUDADataset(Dataset):
    ...
    def process_item(self, img_path, mask_path=None):
        ...
        for c in range(5): mask_binary[:, :, c] = (mask_resized == c).astype(np.float32)
        ...
        mask_long = torch.from_numpy(mask_resized).long()
...
# ==============================================================================
# 7. ROBUST METRICS & 3D POST-PROCESSING (LCC)
# ==============================================================================
def keep_largest_connected_component(mask_volume):
    """
    🌟 THE LCC POST-PROCESSING FIX: 
    Scans the 3D volume for each organ class and physically deletes isolated 
    false-positive specks. This algorithm will drop HD95 scores from 150mm down to < 5mm.
    """
    out_mask = np.zeros_like(mask_volume)
    for c in range(1, 5):  # Classes 1 to 4 (Liver, RK, LK, Spl)
        ...
        # Find the largest connected mass
...
def evaluate_3d_metrics(model, dataloader, device):
    ...
    for pid in patient_preds.keys():
        ...
        # 🌟 APPLY 3D POST-PROCESSING TO REMOVE HALLUCINATIONS
...
def save_qualitative_results(G, S, dataset, device, save_dir, epoch):
    ...
    G.eval(); S.eval()
    with torch.no_grad():
        fake_ct = G(s_img_fda.unsqueeze(0).to(device))
        _, _, p_final, _ = S(fake_ct)
    ...
    c_img, c_fda, f_ct, r_ct = to_img(s_img), to_img(s_img_fda), to_img(fake_ct), to_img(t_img)
    ...
    ax[0,0].imshow(c_img, cmap='gray'); ax[0,0].set_title("Source MR (AMOS22)")
    ax[0,1].imshow(c_fda, cmap='gray'); ax[0,1].set_title("FDA Transformed MR")
    ax[0,2].imshow(f_ct, cmap='gray'); ax[0,2].set_title("Generator Fake CT")
    ax[0,3].imshow(overlay(f_ct, gt)); ax[0,3].set_title("Fake CT + GT Mask")
    
    ax[1,0].imshow(r_ct, cmap='gray'); ax[1,0].set_title("Real CT (AMOS Unseen)")
    ax[1,1].imshow(overlay(f_ct, p_fake)); ax[1,1].set_title("Seg Pred on Fake CT")
    ax[1,2].imshow(overlay(r_ct, p_real)); ax[1,2].set_title("Seg Pred on Real CT")
    ax[1,3].axis('off')
    [a.axis('off') for a in ax.flatten()]
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, f"AMOS_MR2CT_GST_Qualitative_{epoch}.png"), dpi=300)
    plt.close()
    G.train(); S.train()

# ==============================================================================
# 8. MAIN EXECUTION (MR -> CT Phase 2 GST)
# ==============================================================================
if __name__ == '__main__':
    print(f"\n--- Initializing AMOS22 PHASE 2 GST + POST-PROCESSING (MR -> CT) ---")
    ...
    print(f"\nStarting SOTA GST UDA for {CONFIG['EPOCHS']} Epochs...")
    for epoch in range(CONFIG['EPOCHS']):
        ...
        for batch_idx, (s_img, s_img_fda, s_mask_long, s_mask_onehot, s_edge, s_body, t_img) in enumerate(tqdm(train_loader, leave=False, desc=f"Epoch {epoch+1}")):
            ...
            opt_D_img.zero_grad()
            fake_ct = G(s_img_fda) 
            fake_ct_buffered = fake_ct_buffer.push_and_pop(fake_ct)
            loss_D_img = 0.5 * (criterion_GAN(D_img(t_img), valid) + criterion_GAN(D_img(fake_ct_buffered.detach()), fake))
            loss_D_img.backward()
            opt_D_img.step()
            
            opt_G.zero_grad()
            loss_G = criterion_GAN(D_img(fake_ct), valid) + CONFIG['LAMBDA_STRUCT'] * criterion_Struct(s_img_fda, fake_ct)
            loss_G.backward()
            opt_G.step()
            ...
            p_b_fake, p_e_fake, p_f_fake, feat_fake = S(fake_ct.detach()) 
            ...
            # 🌟 MEAN TEACHER LOGIC: PHASE 2 GST (Softmax + Sigmoid Edges)
            with torch.no_grad():
                _, p_t_edge_teacher, p_t_final_teacher, _ = S_Teacher(t_img)
                
                # 1. Final Mask (Softmax -> Argmax)
                prob_teacher_final = torch.softmax(p_t_final_teacher, dim=1).detach()
                argmax_teacher = torch.argmax(prob_teacher_final, dim=1, keepdim=True)
                pseudo_labels_final = torch.zeros_like(prob_teacher_final).scatter_(1, argmax_teacher, 1.0)
                
                # 2. Edge Mask (Sigmoid, edges are not mutually exclusive)
                prob_teacher_edge = torch.sigmoid(p_t_edge_teacher).detach()
                pseudo_labels_edge = (prob_teacher_edge > 0.5).float()
                
                # Confidence Mask (Based on Final Softmax probability)
                thresh = CONFIG['CONFIDENCE_THRESH']
                max_prob_teacher, _ = torch.max(prob_teacher_final, dim=1, keepdim=True)
                conf_mask = (max_prob_teacher > thresh).float()
                
            _, p_t_edge_student, p_t_final_student, feat_real = S(t_img)
            prob_student_final = torch.softmax(p_t_final_student, dim=1)
            prob_student_edge = torch.sigmoid(p_t_edge_student)
            
            class_weights = torch.tensor([1.0, 5.0, 5.0, 2.0], device=device).view(1, 4, 1, 1)
            
            # --- FINAL MASK LOSS (Strip Bg) ---
            prob_student_organs = prob_student_final[:, 1:, :, :]
            pseudo_labels_organs = pseudo_labels_final[:, 1:, :, :]
            conf_mask_organs = conf_mask.expand_as(prob_teacher_final)[:, 1:, :, :]
            
            bce_final = F.binary_cross_entropy(prob_student_organs, pseudo_labels_organs, reduction='none')
            masked_bce_final = (bce_final * class_weights * conf_mask_organs).sum() / (conf_mask_organs.sum() + 1e-8)
            
            inter_f = (prob_student_organs * pseudo_labels_organs * conf_mask_organs * class_weights).sum(dim=(0, 2, 3))
            # 🌟 BOUNDED UNION FIX
            union_f = ((prob_student_organs + pseudo_labels_organs) * conf_mask_organs * class_weights).sum(dim=(0, 2, 3)) 
            masked_dice_final = 1 - ((2. * inter_f + 1e-8) / (union_f + 1e-8)).mean()
            
            # --- EDGE MASK LOSS (Strip Bg) ---
            prob_edge_organs = prob_student_edge[:, 1:, :, :]
            pseudo_labels_edge_organs = pseudo_labels_edge[:, 1:, :, :]
            
            bce_edge = F.binary_cross_entropy(prob_edge_organs, pseudo_labels_edge_organs, reduction='none')
            masked_bce_edge = (bce_edge * class_weights * conf_mask_organs).sum() / (conf_mask_organs.sum() + 1e-8)
            
            inter_e = (prob_edge_organs * pseudo_labels_edge_organs * conf_mask_organs * class_weights).sum(dim=(0, 2, 3))
            # 🌟 BOUNDED UNION FIX
            union_e = ((prob_edge_organs + pseudo_labels_edge_organs) * conf_mask_organs * class_weights).sum(dim=(0, 2, 3)) 
        ...
        print(f"Ep {epoch+1:03d} | W_GST:{alpha_pseudo:.2f} | L_S:{meter['S']/n:.3f} | L_Pseu:{meter['P']/n:.3f} | Target Val 3D Dice: {v_dice:.4f} (LIV:{d_cls[0]:.2f} RK:{d_cls[1]:.2f} LK:{d_cls[2]:.2f} SPL:{d_cls[3]:.2f})")

        if v_dice > best_dice:
            best_dice = v_dice
            torch.save(S.state_dict(), os.path.join(CONFIG['SAVE_DIR'], "best_AMOS_Segmentor_MR2CT_Phase2_GST.pth"))
            torch.save(G.state_dict(), os.path.join(CONFIG['SAVE_DIR'], "best_AMOS_Generator_MR2CT_Phase2_GST.pth"))
            print("   🌟 New Phase 2 Cross-Dataset Checkpoint Saved!")
            
        if (epoch+1) % 10 == 0: save_qualitative_results(G, S, train_ds, device, CONFIG['SAVE_DIR'], epoch+1)

    print("\n" + "="*70)
    print("🚀 FINAL ZERO-SHOT TEST RESULTS + LCC POST-PROCESSING (AMOS MR -> CT PHASE 2)")
    print("="*70)
    
    test_ds = UnpairedUDADataset(None, None, *filter_t(t_test_c, True), img_size=CONFIG['IMG_SIZE'])
    test_loader = DataLoader(test_ds, batch_size=1, shuffle=False)
    
    S.load_state_dict(torch.load(os.path.join(CONFIG['SAVE_DIR'], "best_AMOS_Segmentor_MR2CT_Phase2_GST.pth"), weights_only=True))
