...
# ==============================================================================
# 1. AMOS22 CONFIGURATION (CT -> MR PHASE 2)
# ==============================================================================
CONFIG = {
    ...
    # 🌟 LOAD PHASE 1 WEIGHTS (CT -> MR)
    'PRETRAINED_SEGMENTOR': r'D:\Nilesh\AMOS22\UDA_CT2MR_Phase1\best_AMOS_Segmentor_CT2MR.pth',
    'SAVE_DIR': r'D:\Nilesh\AMOS22\22 July\UDA_CT2MR_Phase2_GST', 
...
def save_qualitative_results(G, S, dataset, device, save_dir, epoch):
    ...
    ax[1,0].imshow(r_mr, cmap='gray'); ax[1,0].set_title("Real MR (AMOS Unseen)")
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, f"AMOS_CT2MR_GST_Qualitative_{epoch}.png"), dpi=300)
    plt.close()
    G.train(); S.train()

# ==============================================================================
# 8. MAIN EXECUTION (CT -> MR Phase 2 GST)
# ==============================================================================
if __name__ == '__main__':
    print(f"\n--- Initializing AMOS22 PHASE 2 GST + POST-PROCESSING (CT -> MR) ---")
    ...
    for epoch in range(CONFIG['EPOCHS']):
        ...
        if v_dice > best_dice:
            best_dice = v_dice
            torch.save(S.state_dict(), os.path.join(CONFIG['SAVE_DIR'], "best_AMOS_Segmentor_CT2MR_Phase2_GST.pth"))
            torch.save(G.state_dict(), os.path.join(CONFIG['SAVE_DIR'], "best_AMOS_Generator_CT2MR_Phase2_GST.pth"))
    ...
    print("\n" + "="*70)
    print("🚀 FINAL ZERO-SHOT TEST RESULTS + LCC POST-PROCESSING (AMOS CT -> MR PHASE 2)")
    print("="*70)
    
    test_ds = UnpairedUDADataset(None, None, *filter_t(t_test_c, True), img_size=CONFIG['IMG_SIZE'])
    test_loader = DataLoader(test_ds, batch_size=1, shuffle=False)
    
    S.load_state_dict(torch.load(os.path.join(CONFIG['SAVE_DIR'], "best_AMOS_Segmentor_CT2MR_Phase2_GST.pth"), weights_only=True))
