...
# ==============================================================================
CONFIG = {
    'IMG_DIR': r'D:\Nilesh\AMOS22\AMOS22_Data_2D\Images\MR',
    'MASK_DIR': r'D:\Nilesh\AMOS22\AMOS22_Data_2D\Masks\MR',
    'SAVE_DIR': r'D:\Nilesh\GST-UDA\AMOS22\DBSNet_MR_4Organs', 
    'EPOCHS': 50, 
    'BATCH_SIZE': 32, 
    'LR': 3e-4,
    'NUM_WORKERS': 4, # Kept at 4 to prevent Windows paging file memory crashes
...
class AMOS22SubsetDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # 3. Apply Subset Mapping (Flatten ignored organs to 0)
        ...
        # 4. Generate 5-Channel Binary Masks, Edges, and Bodies
...
def save_individual_images(model, dataset, device, epoch, save_dir):
    ...
    save("1_Input_MRI", img_vis, cmap='gray')
...
if __name__ == '__main__':
    ...
    print(f"\n--- Initializing AMOS22 MR Pipeline (4-Organ Subset) ---")
    ...
    for epoch in range(CONFIG['EPOCHS']):
        ...
        if current_dice > best_dice:
            ...
            torch.save(model.state_dict(), os.path.join(CONFIG['SAVE_DIR'], "best_model_MR_4Organs.pth"))
    ...
    model.load_state_dict(torch.load(os.path.join(CONFIG['SAVE_DIR'], "best_model_MR_4Organs.pth"), weights_only=True))

    vol_avg_dice, d_cls, vol_avg_hd, hd_cls, vol_avg_asd, asd_cls = calculate_full_3d_metrics(model, test_ds, device)
    
    print("\n" + "="*60)
    print("FINAL 3D METRICS (4 Major Organs - MR)")
    print("="*60)
