...
# ==============================================================================
# 1. SOTA CONFIGURATION (FDA + SIFA + CBST + Mean Teacher)
# ==============================================================================
CONFIG = {
    'SOURCE_IMG_DIR': r'D:\Nilesh\GST-UDA\MMWHS\MMWHS_Data_2D\Images\MR',
    'SOURCE_MASK_DIR': r'D:\Nilesh\GST-UDA\MMWHS\MMWHS_Data_2D\Masks\MR',
    'TARGET_IMG_DIR': r'D:\Nilesh\GST-UDA\MMWHS\MMWHS_Data_2D\Images\CT',
    'TARGET_MASK_DIR': r'D:\Nilesh\GST-UDA\MMWHS\MMWHS_Data_2D\Masks\CT',
    
    'PRETRAINED_SEGMENTOR': r'D:\Nilesh\GST-UDA\MMWHS\DBSNet_MR_Baseline\best_model_MMWHS_MR.pth',
    'SAVE_DIR': r'D:\Nilesh\GST-UDA\MMWHS\SIFA_FDA_CBST_MR2CT', 
    ...
    # 🌟 NOVELTY: Fourier Domain Adaptation Factor
    'FDA_BETA': 0.05             # The low-frequency window size for texture swapping
}
...
def extract_amp_spectrum(img):
    """Computes Fast Fourier Transform to extract Amplitude and Phase."""
...
def FDA_source_to_target(src_img, trg_img, L=0.05):
    """
    Swaps the low-frequency amplitude of the Source (MR) with the Target (CT).
    This deterministically handles global style transfer without a GAN.
    """
    ...
    # Inject Target Texture into Source Amplitude
...
class UnpairedUDADataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # 🌟 NOVELTY: Online Fourier Domain Adaptation (FDA)
        s_img_fda = FDA_source_to_target(s_img, t_img, L=CONFIG['FDA_BETA'])
        
        # Return Both the Original Source and the FDA augmented Source
...
def save_qualitative_results(G, S, dataset, device, save_dir, epoch):
    ...
    ax[0,0].imshow(c_img, cmap='gray'); ax[0,0].set_title("Source MR")
    ...
    ax[1,0].imshow(r_ct, cmap='gray'); ax[1,0].set_title("Real CT Unseen Target")
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, f"SIFA_FDA_CBST_Qualitative_{epoch}.png"), dpi=300)
    plt.close()
    G.train(); S.train()

# ==============================================================================
# 9. MAIN EXECUTION (MR -> CT)
# ==============================================================================
if __name__ == '__main__':
    print(f"\n--- Initializing SOTA SIFA + FDA + Mean Teacher (MR -> CT) ---")
    ...
    S = DBSNet().to(device)
    print(f"Loading Pre-trained Segmentor: {os.path.basename(CONFIG['PRETRAINED_SEGMENTOR'])}")
    ...
    def lr_lambda(epoch): return 1.0 - max(0, epoch - 25) / float(150 - 75)
    ...
    for epoch in range(CONFIG['EPOCHS']):
        ...
        for batch_idx, (s_img, s_img_fda, s_mask, s_edge, s_body, t_img) in enumerate(tqdm(train_loader, leave=False, desc=f"Epoch {epoch+1}")):
            ...
            opt_D_img.zero_grad()
            # 🌟 NOVELTY: Generator uses FDA image as baseline to fix artifacts
            fake_ct = G(s_img_fda) 
            ...
            # 2. Train Generator
            opt_G.zero_grad()
            # 🌟 FDA enforces structure, allowing the Generator to focus purely on domain texture
            loss_G = criterion_GAN(D_img(fake_ct), valid) + CONFIG['LAMBDA_STRUCT'] * criterion_Struct(s_img_fda, fake_ct)
            loss_G.backward()
            opt_G.step()
            
            # ----------------------------------
            # 3. Train Segmentor & Feature Discriminator (Mean Teacher + CBST)
            # ----------------------------------
            ...
            # Train on Generator's Fake CT output
            ...
            # 🌟 MEAN TEACHER + CBST LOGIC ON REAL TARGET (CT)
            # 1. Pass CLEAN Target through the Frozen Teacher
            with torch.no_grad():
                _, _, p_t_final_teacher, _ = S_Teacher(t_img)
                prob_teacher = torch.sigmoid(p_t_final_teacher).detach()
                
                # 🌟 THE CBST UPGRADE: Teacher generates Hard Pseudo-Labels
                pseudo_labels = (prob_teacher > 0.5).float()
                
                # Teacher only enforces learning on highly confident pixels (>0.85 or <0.15)
                conf_mask = ((prob_teacher > 0.85) | (prob_teacher < 0.15)).float()
                
            # 2. Pass CLEAN Target through the Trainable Student (NO GAUSSIAN NOISE)
            _, _, p_t_final_student, feat_real = S(t_img)
            prob_student = torch.sigmoid(p_t_final_student)
            
            # 3. Masked CBST Loss (BCE + Dice against Teacher's confident labels)
            bce = F.binary_cross_entropy_with_logits(p_t_final_student, pseudo_labels, reduction='none')
            masked_bce = (bce * conf_mask).sum() / (conf_mask.sum() + 1e-8)
            
            inter = (prob_student * pseudo_labels * conf_mask).sum(dim=(0, 2, 3))
            union = ((prob_student + pseudo_labels) * conf_mask).sum(dim=(0, 2, 3))
        ...
        print(f"Ep {epoch+1:03d} | W_CBST:{alpha_pseudo:.2f} | L_S:{meter['S']/n:.3f} | L_Pseu:{meter['P']/n:.3f} | Target Val 3D Dice: {v_dice:.4f} (AA:{d_cls[0]:.2f} LAC:{d_cls[1]:.2f} LVC:{d_cls[2]:.2f} MYO:{d_cls[3]:.2f})")

        if v_dice > best_dice:
            best_dice = v_dice
            torch.save(S.state_dict(), os.path.join(CONFIG['SAVE_DIR'], "best_SIFA_FDA_Segmentor_MR2CT.pth"))
            torch.save(G.state_dict(), os.path.join(CONFIG['SAVE_DIR'], "best_SIFA_FDA_Generator_MR2CT.pth"))
            print("   🌟 New Best SOTA Checkpoint Saved!")
            
        if (epoch+1) % 10 == 0: save_qualitative_results(G, S, train_ds, device, CONFIG['SAVE_DIR'], epoch+1)

    print("\n" + "="*60)
    print("🚀 FINAL ZERO-SHOT TEST RESULTS (SIFA + FDA + CBST: MR -> CT)")
    print("="*60)
    
    test_ds = UnpairedUDADataset(None, None, *filter_t(t_test_c, True), img_size=CONFIG['IMG_SIZE'])
    test_loader = DataLoader(test_ds, batch_size=1, shuffle=False)
    
    S.load_state_dict(torch.load(os.path.join(CONFIG['SAVE_DIR'], "best_SIFA_FDA_Segmentor_MR2CT.pth"), weights_only=True))
