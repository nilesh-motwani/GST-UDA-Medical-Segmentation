...
# ==============================================================================
# 1. SOTA CONFIGURATION (FDA + SIFA + CBST + Mean Teacher) CT -> MR
# ==============================================================================
CONFIG = {
    ...
    'PRETRAINED_SEGMENTOR': r'D:\Nilesh\GST-UDA\MMWHS\DBSNet_CT_Baseline\best_model_MMWHS_CT.pth',
    'SAVE_DIR': r'D:\Nilesh\GST-UDA\MMWHS\SIFA_FDA_CBST_CT2MR', 
    ...
    # ---> ADD THIS LINE <---
    'CONFIDENCE_THRESH': 0.70   # Use 0.70 for soft MR targets. (Use 0.85 for MR->CT)             
}
...
def FDA_source_to_target(src_img, trg_img, L=0.05):
    """Swaps low-frequency amplitude of Source (CT) with Target (MR)."""
...
# ==============================================================================
# 9. MAIN EXECUTION (CT -> MR)
# ==============================================================================
if __name__ == '__main__':
    print(f"\n--- Initializing SOTA SIFA + FDA + Mean Teacher (CT -> MR) ---")
    ...
    def lr_lambda(epoch): return 1.0 - max(0, epoch - 75) / float(150 - 75)
    ...
    for epoch in range(CONFIG['EPOCHS']):
        ...
        for batch_idx, (s_img, s_img_fda, s_mask, s_edge, s_body, t_img) in enumerate(tqdm(train_loader, leave=False, desc=f"Epoch {epoch+1}")):
            ...
            # 🌟 MEAN TEACHER LOGIC ON REAL TARGET (MR)
            ...
            with torch.no_grad():
                ...
                # Teacher only enforces learning on highly confident pixels based on CONFIG
                thresh = CONFIG['CONFIDENCE_THRESH']
                conf_mask = ((prob_teacher > thresh) | (prob_teacher < (1.0 - thresh))).float()
    ...
    print("\n" + "="*60)
    print("🚀 FINAL ZERO-SHOT TEST RESULTS (SIFA + FDA + CBST: CT -> MR)")
    print("="*60)
